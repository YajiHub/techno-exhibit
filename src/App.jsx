...
export default function App() {
  ...
  const [twilioConfig, setTwilioConfig] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : { accountSid: '', authToken: '', fromNumber: '' };
  });

  const addLog = (msg) => setLogs(prev => [msg, ...prev.slice(0, 49)]);

  useEffect(() => { contactService.saveContacts(contacts); }, [contacts]);
  useEffect(() => { localStorage.setItem('twilio_config', JSON.stringify(twilioConfig)); }, [twilioConfig]);

  // ── SUPABASE REALTIME GPS RECEIVER ─────────────────────────────────────────
  useEffect(() => {
    let channel = null;
        ...
        channel
          ...
          .subscribe((status) => {
            if (status === 'SUBSCRIBED') {
              addLog('[📡] Realtime GPS bridge connected — waiting for phone...');
            } else if (status === 'CHANNEL_ERROR') {
              addLog('[!] Realtime connection error — check Supabase keys');
            }
          });
      } catch (err) {
  ...
  const handleAddContact = (data) => {
    const c = contactService.addContact(data);
    setContacts(p => [...p, c]);
    addLog(`[+] Contact added: ${data.name}`);
  };
        ...
        <div className="space-y-2">
          {Object.keys(PRODUCTS).map(k => (
            <button
              ...
              className={`w-full p-3 rounded-xl border-2 text-left transition-all text-[11px] ${activeTab === k ? 'border-brand bg-brand/5' : 'border-gray-900 hover:border-gray-800'}`}
            >
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 border-b border-gray-800 flex items-center justify-between px-8 bg-black/20">
          <div className="flex gap-3">
            <button
              onClick={() => setSwView('web')}
              className={`text-[9px] font-black tracking-widest px-4 py-1.5 rounded-full border transition-all ${swView === 'web' ? 'bg-brand text-black border-brand' : 'text-gray-500 border-gray-800'}`}
            >Web</button>
            <button
              onClick={() => setSwView('mobile')}
              className={`text-[9px] font-black tracking-widest px-4 py-1.5 rounded-full border transition-all ${swView === 'mobile' ? 'bg-brand text-black border-brand' : 'text-gray-500 border-gray-800'}`}
            >Mobile</button>
          </div>
          {isSOS && (
            <button onClick={() => setIsSOS(false)} className="text-[9px] font-black bg-red-600 px-4 py-1.5 rounded-full animate-pulse">
              Resolve
            </button>
          )}
        </div>
        ...
        <div className="flex-1 flex gap-2 p-2">
          ...
          <div className="flex-1 relative border border-gray-900/50 bg-[radial-gradient(#111_1px,transparent_1px)] [background-size:32px_32px] rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/80 px-3 py-1 rounded-full border border-gray-800 text-[8px] font-mono text-brand uppercase tracking-widest">
          ...
          <div className="flex-1 flex items-center justify-center bg-[#050505] rounded-lg border border-gray-900/50">
            {swView === 'mobile' ? (
              <MobileSimulator
                isSOS={isSOS} isTrack={isTrack} geo={geo} contacts={contacts}
                onAddContact={handleAddContact} onDeleteContact={handleDeleteContact}
                twilioConfig={twilioConfig} onUpdateTwilio={setTwilioConfig}
              />
            ) : (
              <WebSimulator
